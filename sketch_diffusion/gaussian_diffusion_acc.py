...
class GaussianDiffusion_acc:
    ...
    def ddim_sample(
        ...
        pen_state = th.argmax(pen_state, dim=1)
        pen_state = pen_state.reshape(B, Nmax, -1)
        sample_all_cur = th.cat((sample, pen_state), 2)
        sample_all_cur = bin_pen(sample_all_cur)
        right_num = 0
        for i in range(sample_all_cur.size()[0]):
            sketch_cv = draw.draw_three(np.array(sample_all_cur[i].cpu()))
            sketch_pil = Image.fromarray(sketch_cv)
            sketch_input = preprocess(sketch_pil)
            sketch_input = sketch_input.unsqueeze(0)
            result = acc_model(sketch_input.cuda())
            value, predicted = th.topk(result.softmax(1), sample_all_cur.size()[0])
            if right_answer in predicted[0, :1]:
                right_num += 1
